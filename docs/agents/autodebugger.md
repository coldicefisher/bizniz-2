...
`bizniz/agents/debugger/quick.py` (with backward-compat shim at `bizniz/autodebugger/autodebugger.py` exposing `Autodebugger` as an alias for `QuickDebugger`).
...
### `diagnose(error_output, code, code_filename, test_code, test_filename) → AutodebuggerDiagnosis`
...
4. Single LLM call with `AutodebuggerSchema`.
5. Parse `relevant_files` (handles both the array-of-objects and the dict-shape responses) and return `AutodebuggerDiagnosis`.
...
# Or: from bizniz.autodebugger.autodebugger import Autodebugger
...
- **Calls into:** `BaseAIClient.get_text` (`AutodebuggerSchema`), `BaseWorkspace.{list_relative_files, read_file}`.
...
- **`Autodebugger` is just an alias** kept for backward compatibility — new code should import `QuickDebugger`.
