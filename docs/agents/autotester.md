...
`bizniz/tester/tester.py`. Generates and reviews test suites.

## Purpose

The tester writes test files for code the coder produces. It supports three single-file modes plus a multi-file tool-loop mode that mirrors the coder's `generate_multi`.
...
|------|--------|-------------|
| 1 | `process_from_code(code_path, output_path)` | Read existing code at `code_path`, look up its problem statement (workspace DB → file metadata), ask AI for tests. Used after the coder has already produced source. |
...
All three return `TesterResult(test_files=[GeneratedTestFile(filepath, tests)], mode, success)`.

### Multi-file mode

`generate_multi(problem_statement, test_files, source_code=None, architecture_context="") → TesterResult`

Uses `bizniz.tools.tool_loop.run_tool_loop` with `TesterGenerateActionSchema` and terminal action `submit_tests`. Small source files (under 4000 chars) are inlined in the prompt; larger ones are listed as "use `view_file` to read".
...
| `_generate_tests(user_prompt, mode)` | 3-attempt LLM call with `TesterSchema`. Handles both new (`test_files: [{filepath, tests}]`) and old (`tests: "..."`) response shapes. Fixes double-escaped newlines. |
...
- **Calls into:** `BaseAIClient.get_text` (`TesterSchema`), `run_tool_loop` (`TesterGenerateActionSchema`), `workspace.read_file/write_file/db.get_context_for_code_file`, `bizniz.utils.code_metadata.read_code_metadata`.
...
- **`process_from_prompt` doesn't read the source.** It writes contract tests against an interface only. If you want the tester to look at existing code, use `process_from_code` or `review_tests`.
...
- **`success=True` in the result is set unconditionally if no exception is raised.** A successful AI response with empty tests will fail at `_save_tests` (and the tester still raises). But "success" doesn't mean the tests pass — only that a file was generated and saved.
