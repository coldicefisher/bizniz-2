...
from bizniz.engineer.prompts.system_prompt import ENGINEER_SYSTEM_PROMPT, get_engineer_system_prompt
...
class Engineer(BaseAIAgent):
    ...
    def analyze(self, problem_statement: str) -> EngineeringAnalysis:
        ...
        log("Engineer: saving problem statement...")
        ...
        log("Engineer: persisting analysis to workspace DB...")
        analysis = self._persist_analysis(problem_id, raw)

        # Step 2: Architecture planning
        log("Engineer: planning architecture...")
        ...
        log("Engineer: refining issues with architecture context...")
    ...
    def dispatch(
        ...
        log(f"Engineer: dispatching orchestrator for issue #{issue_id} — {row['title']}")
        ...
        log(f"Engineer: issue #{issue_id} failed with CODE_FIRST — retrying with TDD...")
        result = self._run_orchestrator(
            row=row,
            target_files=target_files,
            test_files=test_files,
        ...
        log(f"Engineer: issue #{issue_id} failed with TDD — re-prompting...")
        ...
        log(f"Engineer: issue #{issue_id} — trying scope reduction...")
    ...
    def _run_orchestrator(
        ...
        # Append test setup hint if available — helps tester write correct test scaffolding
        ...
        try:
            ...
            log(f"Engineer: hit max iterations with {strategy.value}")
            ...
            log("Engineer: API account has insufficient funds — stopping.")
            raise
        except Exception as e:
            log(f"Engineer: crashed ({type(e).__name__}: {e})")
            return OrchestratorResult(
                success=False,
                changes=[],
                test_files=[],
                ...
                try:
                    ...
                    drift_items = [
                        DriftItem(
                            ...
                            reason="File created by coder but not in architecture plan",
                        )
                        for fp in result.drift_files
                    ]
                    decision = self.review_drift(plan, drift_items)
                    if decision.decision == "approve":
                        log(f"Engineer: drift approved — {decision.reason}")
                    elif decision.decision == "modify":
                        log(f"Engineer: plan modified — {decision.reason}")
                    elif decision.decision == "reject":
                        log(f"Engineer: drift rejected — {decision.reason}")
                except Exception as e:
                    log(f"Engineer: governance review failed — {e}")

        if result.success:
            self._workspace.db.close_issue(issue_id)
            log(f"Engineer: issue #{issue_id} closed successfully.")
        else:
            self._workspace.db.update_issue_status(issue_id, "open")
            log(f"Engineer: issue #{issue_id} could not be resolved — reset to open.")
        return result
    ...
    def _reprompt_issue(self, row, result, log) -> Optional[str]:
        ...
        try:
            ...
            if text and text.strip():
                log(f"Engineer: re-prompted issue — new description ({len(text)} chars)")
                return text.strip()
        except AIInsufficientFunds:
            raise
        except Exception as e:
            log(f"Engineer: re-prompting failed — {e}")
        return None
    ...
    def _reduce_scope(self, row, result, log) -> Optional[str]:
        ...
        try:
            ...
            if text and text.strip():
                log(f"Engineer: reduced scope — new description ({len(text)} chars)")
                return text.strip()
        except AIInsufficientFunds:
            raise
        except Exception as e:
            log(f"Engineer: scope reduction failed — {e}")
        return None
    ...
    def run_layered(
        ...
        try:
            ...
            log(f"Engineer: {e} — falling back to sequential dispatch")
            return self.run(problem_statement)

        log(f"Engineer: {len(analysis.issues)} issues sorted into {len(layers)} layer(s)")
        for layer in layers:
        ...
        for layer in layers:
            log(
                f"Engineer: dispatching layer {layer.layer_index} "
    ...
    def run_three_phase(
        ...
        log(
            f"Engineer: three-phase strategy — "
        ...
        for model in escalation_models:
            if not failing_ids:
                break
            log(
                f"Engineer: Phase 2 pass with model '{model}' — "
        ...
        if failing_ids and debugger_model:
            log(
                f"Engineer: Phase 3 (agentic debug, model '{debugger_model}', "
        ...
        log(
            f"Engineer: three-phase complete — "
    ...
    def _run_single_attempt(
        ...
        try:
            ...
            log(
                f"Engineer: orchestrator factory failed for issue "
    ...
    def _frame_all_issues(self, analysis: EngineeringAnalysis, layers) -> None:
        ...
        coder once with no tests / no Docker to populate the workspace
        with real baseline code. Borrows an coder from a fresh
        ...
        try:
            ...
            log(f"Engineer: skipping framing — orchestrator factory failed: {e}")
            return

        coder = getattr(framing_orchestrator, "_coder", None)
        if coder is None:
            log("Engineer: skipping framing — no coder on orchestrator")
            return
        ...
        try:
            ...
            log(f"Engineer: framing pass aborted ({type(e).__name__}: {e})")
    ...
    def _dispatch_layer(
        ...
        log(
            f"Engineer: layer {layer.layer_index} — "
    ...
    def plan_architecture(
        ...
        log("Engineer: persisting architecture plan...")
        plan = self._persist_architecture_plan(problem_id, raw)
        log(
            f"Engineer: architecture plan complete — "
    ...
    def create_package_structure(self, plan: ArchitecturePlan):
        ...
        (e.g. models/, api/) are NOT pre-created because the coder may generate
    ...
    def review_drift(
        ...
        log("Engineer: reviewing architecture drift...")
        ...
        log(f"Engineer: governance decision — {decision.decision}: {decision.reason}")
        ...
        if decision.decision == "modify" and decision.plan_updates and plan.db_id:
            try:
                ...
                log("Engineer: architecture plan updated.")
            except Exception as e:
                log(f"Engineer: failed to apply plan updates — {e}")
    ...
    def _backfill_test_setup_hints(
        ...
        if backfilled:
            log(f"Engineer: backfilled test_setup_hint for {backfilled} integration issue(s)")
    ...
    def _call_ai_for_plan(self, user_prompt: str) -> dict:
        ...
        for attempt in range(1, attempts + 1):
            try:
                log(f"Engineer: AI plan call (attempt {attempt}/{attempts})...")
                t0 = time.time()
                ...
                log(f"Engineer: AI plan responded in {elapsed:.1f}s ({len(text or '')} chars)")
                self.add_messages_to_history(output_messages)

                if not text or not text.strip():
                    last_error = "Empty response from AI"
                    log(f"Engineer: empty plan response on attempt {attempt}")
                    continue
                ...
                last_error = e
                log(f"Engineer: plan attempt {attempt} failed — {type(e).__name__}: {e}")
                continue

        raise EngineerBadAIResponseError(
    ...
    def _call_ai_for_analysis(self, user_prompt: str) -> dict:
        ...
        for attempt in range(1, attempts + 1):
            try:
                log(f"Engineer: AI analysis call (attempt {attempt}/{attempts})...")
                t0 = time.time()
                text, job_id, output_messages = self._client.get_text(
                    messages=self.message_history,
                    response_format=ResponseFormat.JSON_SCHEMA,
                ...
                elapsed = time.time() - t0
                log(f"Engineer: AI responded in {elapsed:.1f}s ({len(text or '')} chars)")
                self.add_messages_to_history(output_messages)

                if not text or not text.strip():
                    last_error = "Empty response from AI"
                    log(f"Engineer: empty response on attempt {attempt}")
                    continue
                ...
                last_error = e
                log(f"Engineer: attempt {attempt} failed — {type(e).__name__}: {e}")
                continue
