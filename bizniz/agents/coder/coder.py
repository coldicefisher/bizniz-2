...
from bizniz.agents.coder.prompts.generate_prompts import GENERATE_SYSTEM_INSTRUCTIONS_PROMPT, GENERATE_RETURN_FORMAT_PROMPT
from bizniz.agents.coder.prompts.repair_prompt import REPAIR_PROMPT
from bizniz.agents.coder.prompts.generate_multi_prompt import (
    GENERATE_MULTI_SYSTEM_PROMPT,
    GENERATE_MULTI_USER_PROMPT_TEMPLATE,
    get_generate_multi_system_prompt,
)
from bizniz.agents.coder.prompts.repair_multi_prompt import (
    REPAIR_MULTI_PROMPT_TEMPLATE,
    REPAIR_MULTI_SYSTEM_PROMPT,
)
from bizniz.agents.coder.prompts.repair_inline_prompt import (
    REPAIR_INLINE_SYSTEM_PROMPT,
    REPAIR_INLINE_USER_PROMPT,
)
from bizniz.agents.coder.prompts.prompt_schemas import GeneratePromptSchema, RepairPromptSchema
from bizniz.agents.coder.prompts.tool_action_schema import (
    CoderGenerateActionSchema,
    CoderRepairActionSchema,
)
from bizniz.tools.tool_loop import run_tool_loop, ToolLoopError
from bizniz.agents.coder.types import (
    CoderProcessError,
    CoderBadAIResponseError,
    CoderProcessResult,
    CoderOnEventCallback,
    FileChange,
)
...
class Coder(BaseAIAgent):
    ...
    def __init__(
        ...
        on_event: Optional[Callable[[CoderOnEventCallback], None]] = None,
    ...
    def generate_only(
        ...
        try:
            ...
            raise CoderProcessError(f"Failed to generate code: {e}") from e
    ...
    def generate(
        ...
        def return_result(original_code: str, output, new_code: str) -> CoderProcessResult:
        ...
        try:
            ...
            raise CoderProcessError(f"Failed to generate initial code: {e}") from e
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if not evaluation.success:
                ...
                if error is None:
                    raise CoderProcessError("Code evaluation failed without providing an error message.")
    ...
    def _generate_code(self, messages: List[Message]) -> tuple:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                self.emit(CoderOnEventCallback(
                    stage="generate",
                    status="success",
                    code=code,
    ...
    def generate_multi(
        ...
        try:
            action = run_tool_loop(
                ...
                action_schema=CoderGenerateActionSchema,
            ...
            raise CoderBadAIResponseError(f"Tool loop failed: {e}")

        changes = self._parse_changes(action)
        if not changes:
            raise CoderBadAIResponseError("Tool loop returned no file changes")
        ...
        if dependencies:
            log(f"Coder: LLM declared dependencies: {', '.join(dependencies)}")
    ...
    def repair_multi(
        ...
        try:
            action = run_tool_loop(
                ...
                action_schema=CoderRepairActionSchema,
                terminal_action="submit_code",
                max_turns=6,
                timeout_seconds=300,
                on_status_message=self._on_status_message,
        ...
        if dependencies:
            log(f"Coder: repair declared dependencies: {', '.join(dependencies)}")

        return CoderProcessResult(changes=changes, dependencies=dependencies)
    ...
    def repair_multi_inline(
        ...
        log(f"Coder: inline repair with {len(source_files)} source + {len(test_files)} test file(s)...")
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                log(f"Coder: inline repair response in {elapsed:.1f}s (attempt {attempt})")
                ...
                if analysis:
                    log(f"Coder: analysis — {analysis[:120]}")
                if fix_plan:
                    log(f"Coder: fix plan — {fix_plan[:120]}")
                ...
                log(f"Coder: inline repair JSON error (attempt {attempt}): {e}")
            except AIInsufficientFunds:
                raise
            except Exception as e:
                last_error = f"{type(e).__name__}: {e}"
                log(f"Coder: inline repair error (attempt {attempt}): {e}")

        raise CoderBadAIResponseError(f"Inline repair failed after {attempts} attempts: {last_error}")
    ...
    def _generate_multi_code(self, user_prompt: str) -> tuple:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                elapsed = time.time() - t0
                log(f"Coder: generate_multi AI responded in {elapsed:.1f}s ({len(text or '')} chars)")
                self.add_messages_to_history(output_messages)

                if not text or not text.strip():
                    last_error = "Empty response from AI"
                    log(f"Coder: empty response on attempt {attempt}")
                    continue
                ...
                if not changes:
                    ...
                    log(f"Coder: no changes in response on attempt {attempt}")
                    continue

                dependencies = json_response.get("dependencies", [])
                return changes, dependencies

            except CoderBadAIResponseError:
                raise
            except AIInsufficientFunds:
                raise
            except Exception as e:
                last_error = e
                log(f"Coder: generate attempt {attempt} failed — {type(e).__name__}: {str(e)[:200]}")
                if "Expecting" in str(e) or "json" in str(e).lower():
                    log("Coder: clearing message history due to parse error (prevent token bloat)")
    ...
    def _repair_multi_code(self, repair_prompt: str) -> tuple:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                elapsed = time.time() - t0
                log(f"Coder: repair_multi AI responded in {elapsed:.1f}s ({len(text or '')} chars)")
                self.add_messages_to_history(output_messages)

                if not text or not text.strip():
                    last_error = "Empty response from AI"
                    log(f"Coder: empty repair response on attempt {attempt}")
                    continue
                ...
                if not changes:
                    ...
                    log(f"Coder: no changes in repair response on attempt {attempt}")
                    continue
                ...
                last_error = e
                log(f"Coder: repair attempt {attempt} failed — {type(e).__name__}: {str(e)[:200]}")
