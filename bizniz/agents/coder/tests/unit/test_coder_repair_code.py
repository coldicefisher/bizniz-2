...
from bizniz.agents.coder.types import CoderBadAIResponseError
from bizniz.environment.types import ExecutionCallSpec

from bizniz.agents.coder.tests.conftest import make_get_text_response, VALID_REPAIR_JSON


def test_repair_code_returns_code_and_call_spec(coder, mock_client):
    mock_client.get_text.return_value = make_get_text_response(VALID_REPAIR_JSON)

    code, call_spec = coder._repair_code(
...
def test_repair_code_adds_repair_prompt_to_history(coder, mock_client):
    mock_client.get_text.return_value = make_get_text_response(VALID_REPAIR_JSON)
    history_before = len(coder._message_history)
    ...
    assert len(coder._message_history) > history_before


def test_repair_code_prompt_contains_error_and_code(coder, mock_client):
    mock_client.get_text.return_value = make_get_text_response(VALID_REPAIR_JSON)
...
def test_repair_code_retries_on_invalid_json(coder, mock_client):
...
def test_repair_code_raises_after_exhausted_retries(coder, mock_client):
...
def test_repair_code_raises_on_empty_code(coder, mock_client):
    empty_code_response = {
        "code": "",
        "analysis": "nothing",
        "fix_plan": "nothing",
