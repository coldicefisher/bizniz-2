...
def test_strip_code_block_plain_text(coder):
    assert coder._strip_code_block("print('hello')") == "print('hello')"


def test_strip_code_block_python_fence(coder):
    text = "```python\nprint('hello')\n```"
    assert coder._strip_code_block(text) == "print('hello')"


def test_strip_code_block_generic_fence(coder):
    text = "```\nprint('hello')\n```"
    assert coder._strip_code_block(text) == "print('hello')"
...
def test_save_code_creates_file(real_workspace_autocoder):
    coder, ws, tmp_path = real_workspace_autocoder

    coder._save_code_to_file(code="print('hello')", filename="output.py")
...
def test_save_code_backs_up_existing_file(real_workspace_autocoder):
    ...
    coder._save_code_to_file(code="new code", filename="out.py")
...
def test_save_code_sanitizes_cached_filename(real_workspace_autocoder):
    ...
    coder._save_code_to_file(code="x = 1", filename="bad:name?.py")
