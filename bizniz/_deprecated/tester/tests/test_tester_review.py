...
def test_review_returns_result(tester, mock_workspace):
    mock_workspace.read_file.side_effect = [CODE, EXISTING_TESTS]

    result = tester.review_tests(
...
def test_review_reads_both_code_and_tests(tester, mock_workspace):
    mock_workspace.read_file.side_effect = [CODE, EXISTING_TESTS]
...
def test_review_passes_existing_tests_to_ai(tester, mock_client, mock_workspace):
    mock_workspace.read_file.side_effect = [CODE, EXISTING_TESTS]
...
def test_review_saves_improved_tests(tester, mock_workspace):
    mock_workspace.read_file.side_effect = [CODE, EXISTING_TESTS]
