"""
The old _ai_verify_code step has been removed from the refactored Coder.
...
from bizniz.agents.coder.types import CoderOnEventCallback
...
def test_emit_calls_on_event_callback(coder, mock_environment):
    ...
    assert len(events) > 0
    assert all(isinstance(e, CoderOnEventCallback) for e in events)


def test_generate_stage_event_fired_on_success(coder, mock_environment):
    mock_environment.execute.return_value = ExecutionEnvironmentResult(
        success=True, result=1
    )
    events = []
...
def test_emit_no_callback_does_not_raise(coder, mock_environment):
    mock_environment.execute.return_value = ExecutionEnvironmentResult(
        success=True, result=0
    )
    # No on_event provided — should not raise
    coder.generate(prompt="test", filename="test.py")
