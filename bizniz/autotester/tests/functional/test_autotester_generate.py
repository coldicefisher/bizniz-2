"""
Functional tests: verify Autotester can generate tests via real API calls.

Run with:
    pytest bizniz/autotester/tests/functional/ -m functional -v
"""
import pytest
...
def test_generate_tests_from_prompt(api_key, workspace_path):
    """Autotester generates tests from a problem statement."""
    ...
    autotester = Autotester(client=client, environment=environment, workspace=workspace)
...
def test_generate_multi_tests(api_key, workspace_with_code):
    """Autotester generates tests for multiple source files."""
    config = BiznizConfig(api_key=api_key)
    client = config.make_client()
