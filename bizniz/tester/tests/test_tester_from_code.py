...
def test_from_code_returns_result(tester, mock_workspace):
    mock_workspace.read_file.return_value = CODE_WITH_METADATA

    result = tester.process_from_code(
        code_path="add.py",
...
def test_from_code_reads_code_from_workspace(tester, mock_workspace):
    mock_workspace.read_file.return_value = CODE_WITH_METADATA
    tester.process_from_code(code_path="add.py", output_path="test_add.py")
    mock_workspace.read_file.assert_called_with(path="add.py")


def test_from_code_extracts_problem_statement_for_ai(tester, mock_client, mock_workspace):
    mock_workspace.read_file.return_value = CODE_WITH_METADATA
    tester.process_from_code(code_path="add.py", output_path="test_add.py")
...
def test_from_code_falls_back_when_no_metadata(tester, mock_client, mock_workspace):
    mock_workspace.read_file.return_value = CODE_WITHOUT_METADATA
    # Should not raise — falls back to "(no problem statement found)"
    result = tester.process_from_code(code_path="add.py", output_path="test_add.py")
    assert result.success is True


def test_from_code_saves_tests_to_workspace(tester, mock_workspace):
    mock_workspace.read_file.return_value = CODE_WITH_METADATA
    tester.process_from_code(code_path="add.py", output_path="test_add.py")
