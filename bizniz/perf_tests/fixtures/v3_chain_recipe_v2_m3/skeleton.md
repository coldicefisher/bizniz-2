...
|---|---|---|
| `app/api/routes/<feature>.py` | `app/api/routes/auth.py` | yes — `app/main.py` mounts every router under `/api/v1` |
| `app/models/<feature>.py` | `app/models/user.py` | yes — `app/models/__init__.py` imports every module so SQLAlchemy registers tables |
| `app/schemas/<feature>.py` | `app/schemas/auth.py` | no — import explicitly where used |
| `app/services/<feature>.py` | (none) | no — business-logic helpers |
