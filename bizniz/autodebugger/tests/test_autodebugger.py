...
from bizniz.autodebugger.types import AutodebuggerDiagnosis, AutodebuggerBadAIResponseError
...
def test_diagnose_returns_diagnosis(mock_client, mock_environment, mock_workspace):
    ...
    assert isinstance(result, AutodebuggerDiagnosis)
...
def test_diagnose_raises_after_max_retries(mock_client, mock_environment, mock_workspace):
    ...
    with pytest.raises(AutodebuggerBadAIResponseError):
        debugger.diagnose(
            error_output="error",
            code="pass",
            code_filename="foo.py",
